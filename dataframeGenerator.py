...
d = {'C1': pd.Series([random.randint(1,10000) for n in range(N)], index=[n for n in range(N)]),
     'C2': pd.Series([random.randint(1,10000) for n in range(N)], index=[n for n in range(N)]),
...
dframe.to_csv('/home/piotr/Desktop/PracaInzynierska/losowe_liczby.csv', index=False)